...
namespace compto {
	...
	public static class complib {
		...
		public static int Encode(int version, byte[] @in, int inl, byte[] @out, ref uint outl) {
			/* pointers! */ int insp = 0, inst = 0, ousp = 0, oust = 0, inspb = 0, insplb = 0;
		...
		public static int Decode(int version, byte[] @in, uint inl, byte[] @out, ref uint outl) {
			...
			for (;;) {
				...
				if (profilef != null) profilef.Write("REPEAT[{0:X3},{1:X2}] : (", i + 1, j); for (k = 0; k <= i; k++) { { { if (ousp >= oust) { error = SUCCESS; goto _cleanup; } @out[ousp++] = (byte)j; } State.text_buf[r++] = (byte)j; r &= (uint)(N - 1); };  if (profilef != null && k != 0) profilef.Write(", "); if (profilef != null) profilef.Write("{0:X2}", j); } if (profilef != null) profilef.WriteLine(")");
			}
